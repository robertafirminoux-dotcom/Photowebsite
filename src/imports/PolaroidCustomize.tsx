...
interface PolaroidCustomizeProps {
...
function Frame2({ selected, onClick }: { selected: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="bg-white h-[150px] relative rounded-[12px] shrink-0 w-[120px] md:w-[148px] cursor-pointer hover:opacity-80 transition-opacity"
    >
      {selected && (
...
function Frame1({ selected, onClick }: { selected: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="bg-[#080b12] h-[150px] rounded-[12px] shrink-0 w-[120px] md:w-[148px] cursor-pointer hover:opacity-80 transition-opacity relative"
    >
      {selected && (
        <div aria-hidden="true" className="absolute border-4 border-[#e82ebf] border-solid inset-0 pointer-events-none rounded-[12px]" />
      )}
...
function Frame7({ selectedBg, onBgSelect, noteText, onNoteChange, onContinue }: { 
  ...
  return (
    <div className="w-full px-4 md:px-0 md:absolute md:left-[107px] md:top-[134px] md:w-[715px] content-stretch flex flex-col gap-[24px] items-start py-8 md:py-0">
...
export default function PolaroidCustomize({ onNavigate }: PolaroidCustomizeProps) {
  const [selectedBg, setSelectedBg] = useState<'white' | 'black' | 'colorful'>('white');
  const [noteText, setNoteText] = useState('');

  const handleContinue = () => {
    onNavigate('polaroid-result');
  };
